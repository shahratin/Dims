...
void builddata(gchar *datafilename, glong &wordcount, glong &idxfilesize, glong &synwordcount, std::list<std::string> *TagList, std::list<std::string> *ElementList)
{
	...
	sprintf(idxfilename, "powerword_%s.idx", basefilename);
	sprintf(dicfilename, "powerword_%s.dict", basefilename);
	...
	if (array2->len) {
                ...
                sprintf(synfilename, "powerword_%s.syn", basefilename);
...
void buildifo(gchar *xmlfilename, glong wordcount, glong idxfilesize, glong synwordcount)
{
	...
	gchar *basefilename = g_strdup(xmlfilename);
        ...
        sprintf(ifofilename, "powerword_%s.ifo", basefilename);
	...
	gchar command[256];
        sprintf(command, "mkdir stardict-powerword_%s-2.4.2", basefilename);
        system(command);
	sprintf(command, "mv powerword_%s.idx stardict-powerword_%s-2.4.2", basefilename, basefilename);
	system(command);
	if (synwordcount) {
		sprintf(command, "mv powerword_%s.syn stardict-powerword_%s-2.4.2", basefilename, basefilename);
        	system(command);
	}
	sprintf(command, "mv powerword_%s.dict.dz stardict-powerword_%s-2.4.2", basefilename, basefilename);
        system(command);
	sprintf(command, "mv powerword_%s.ifo stardict-powerword_%s-2.4.2", basefilename, basefilename);
	system(command);
	sprintf(command, "chmod 644 stardict-powerword_%s-2.4.2/*", basefilename);
	system(command);
...
bool convert_utf(gchar *filename, gchar *utffilename)
{
	...
	if (strcmp(utffilename, "pwdnnjsj.da3.utf")==0) {
		...
		system(command);
	} else if (strcmp(utffilename, "pwdecahd.da3.utf")==0) {
		g_print("You need to edit \"affair\" and \"apparent\" manually, add end tags.\n");
	} else if (strcmp(utffilename, "pwzeczl.da3.utf")==0) {
		g_print("You need to edit \"construction of railways\", \"preparation\", \"vehicle\", \"details\", \"mining\" and \"discharge tube\" manually, add end tags.\n");
	}
